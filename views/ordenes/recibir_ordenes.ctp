      ...
      <span class="input-group-btn">
       ...
       ?>
        ...
        <ul class="nav nav-tabs" role="tablist">
          <li role="presentation" class="active"><a href="#datosordendo" aria-controls="datosordendo" role="tab" data-toggle="tab">Ordenes Drop-Off y A Domicilio</a></li>
          <li role="presentation"><a href="#datosordenper" aria-controls="datosordenper" role="tab" data-toggle="tab">Ordenes  Entrega En Persona</a></li>
...
if(isset($Error)){echo $cargar->msj_error($Error);}
 ...
 ?>
          <div role="tabpanel" class="tab-pane active" id="datosordendo">


    <div  style="border:1px">
     <table id='tblMain2' class="table table-bordered table-hover" cellpadding="0" cellspacing="0"  width="100%">
	<thead><tr>
...
if(isset($Exito)){echo $cargar->msj_exito($Exito);}
if(isset($Error)){echo $cargar->msj_error($Error);}
$paginator->options(array('url'=>array( 'controller' => 'Ordenes', 'action' => 'recibir_ordenes_personal'),'update' => 'datosordenper', 'indicator' => 'mini_loading','loading'=>'mini_loading'));

?>         
<div role="tabpanel" class="tab-pane" id="datosordenper">
    <div  style="border:1px">
 ...
 $('#qr-canvas').WebCodeCam({
            ...
            resultFunction: function(resText, lastImageSrc) {
                        ...
                        $.ajax({
                                ...
                                $("#recibeorden").find(".panel-body").children().remove();
                                $("#recibeorden").hide(1000);
                                $("#ordenarticulo").removeClass("animated zoomOut");
                                $("#ordenarticulo").fadeIn(500);
                                $('#ordenarticulo .panel-body').html(data); 
...
<script type="text/javascript">
      ...
      $('#modal_qrcode').modal('show');
            ...
            $('#busquedaorden').focus();      
$('#tbordend tr').on('click', function(){ 
    var row=$(this).parent();
    
    var orden = $("#tbordend").children().eq($(this).index());  
    ...
    $.ajax({
        ...
        complete:function(request, json) {
            $('#ordenarticulo .panel-body').html(request.responseText); 
            $('#mini_loading').hide();
        },
        url:'Ordenes/recibirordenes/'+id}) });

$('#tbordendrop tr').on('click', function(){ 
    var row=$(this).parent();
    
    var orden = $("#tbordendrop").children().eq($(this).index());  
    var id = orden.attr('id');
    $.ajax({
        async:true, 
        type:'post', 
