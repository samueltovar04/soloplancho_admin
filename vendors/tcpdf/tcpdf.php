	...
	*/
	class TCPDF {
		...
		public function setPageFormat($format, $orientation='P') {
			...
			if (is_string($format)) {
				...
				switch (strtoupper($format)) {
					...
					case 'B8': {$format = array(192.75,266.45); break;}
					...
					case 'RA3': {$format = array(752.00,1008.00); break;}
		...
		* @since 2.0.000 (2008-01-03)
		...
		public function getTra_seme($p) {
		
		$di=array(0=>'Inicial','Primer','Segundo','Tercer','Cuarto','Quinto','Sexto','Septimo','Octavo','Noveno','Decimo');
		return $di[$p];
		}
		public function getDia($p) {
		if($p < 10) $p=substr($p,1,1);
		$di=array('Cero','Un','Dos','Tres','Cuatro','Cinco','Seis','Siete','Ocho','Nueve','Diez','Once','Doce','Trece','Catorce','Quince','Dieciseis','Diecisiete','Dieciocho','Diecinueve','Veinte','Veintiun','Veintidos','Veintitres','Veinticuatro','Veinticinco','Veintiseis','Veintisiete','Ventiocho','Ventinueve','Trenta','Trenta y un');
		return $di[$p];
		}
		public function getMes($p) {
		if($p < 10) $p=substr($p,1,1);
		$di=array('Cero','Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');
		return $di[$p];
		}
		public function letra($p) {
		
		$di=array('CERO','UNO','DOS','TRES','CUATRO','CINCO','SEIS','SIETE','OCHO','NUEVE','DIEZ','ONCE','DOCE', 'TRECE','CATORCE', 'QUINCE', 'DIECISEIS', 'DIECISIETE', 'DIECIOCHO', 'DIECINUEVE','VEINTE',
'VEINTIUNO', 'VEINTIDOS', 'VEINTITRES', 'VEINTICUATRO','VEINTICINTO', 'VEINTISEIS', 'VEINTISIETE', 'VEINTIOCHO', 'VEINTINUEVE','TREINTA','TREINTA Y UNO', 'TREINTA Y DOS', 'TREINTA Y TRES', 'TREINTA Y CUATRO', 
'TREINTA Y CINCO', 'TREINTA Y SEIS', 'TREINTA Y SIETE', 'TREINTA Y OCHO', 'TREINTA Y NUEVE','CUARENTA','CUARENTA Y UNO', 'CUARENTA Y DOS', 'CUARENTA Y TRES', 'CUARENTA Y CUATRO', 'CUARENTA Y CINCO', 'CUARENTA Y SEIS',
'CUARENTA Y SIETE', 'CUARENTA Y OCHO', 'CUARENTA Y NUEVE','CINCUENTA','CINCUENTA Y UNO', 'CINCUENTA Y DOS', 'CINCUENTA Y TRES', 'CINCUENTA Y CUATRO', 'CINCUENTA Y CINCO', 'CINCUENTA Y SEIS', 'CINCUENTA Y SIETE',
'CINCUENTA Y OCHO', 'CINCUENTA Y NUEVE','SESENTA','SESENTA Y UNO', 'SESENTA Y DOS', 'SESENTA Y TRES', 'SESENTA Y CUATRO', 'SESENTA Y CINCO', 'SESENTA Y SEIS', 'SESENTA Y SIETE', 'SESENTA Y OCHO', 'SESENTA Y NUEVE',
'SETENTA','SETENTA Y UNO', 'SETENTA Y DOS', 'SETENTA Y TRES', 'SETENTA Y CUATRO', 'SETENTA Y CINCO', 'SETENTA Y SEIS', 'SETENTA Y SIETE', 'SETENTA Y OCHO', 'SETENTA Y NUEVE','OCHENTA','OCHENTA Y UNO', 'OCHENTA Y DOS', 
...
'NOVENTA Y CINCO', 'NOVENTA Y SEIS', 'NOVENTA Y SIETE', 'NOVENTA Y OCHO', 'NOVENTA Y NUEVE','CIEN');
		return $di[$p];
		}
		/**
...
: '.$ctgfile);
		...
		*/
		 ...
		 * @access public
		 */		
		public function conversisgemColorToDec($color='#FFFFFF') {
			global $webcolor;
			$returncolor = false;
...
: '.$file);
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		 ...
		 */
		protected function gesisgemDomArray($html) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{0} == '/') {
						...
						if (isset($dom[$key]['attribute']['style'])) {
							...
							if (isset($dom[$key]['style']['font-size'])) {
								...
								switch ($fsize) {
									...
									default: {
										$dom[$key]['fontsize'] = $this->gesisgemUnitToUnits($fsize, $dom[$parentkey]['fontsize'], 'pt', true);
									}
								}
							}
							// font style
							...
							if (isset($dom[$key]['style']['color']) AND (!$this->empty_string($dom[$key]['style']['color']))) {
								$dom[$key]['fgcolor'] = $this->conversisgemColorToDec($dom[$key]['style']['color']);
							}
							// background color
							if (isset($dom[$key]['style']['background-color']) AND (!$this->empty_string($dom[$key]['style']['background-color']))) {
								$dom[$key]['bgcolor'] = $this->conversisgemColorToDec($dom[$key]['style']['background-color']);
							}
							// text-decoration
						...
						if (isset($dom[$key]['attribute']['color']) AND (!$this->empty_string($dom[$key]['attribute']['color']))) {
							$dom[$key]['fgcolor'] = $this->conversisgemColorToDec($dom[$key]['attribute']['color']);
						}
						// set background color attribute
						if (isset($dom[$key]['attribute']['bgcolor']) AND (!$this->empty_string($dom[$key]['attribute']['bgcolor']))) {
							$dom[$key]['bgcolor'] = $this->conversisgemColorToDec($dom[$key]['attribute']['bgcolor']);
						}
						// check for width attribute
		...
		public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
			...
			$dom = $this->gesisgemDomArray($html);
			$maxel = count($dom);
			$key = 0;
			while ($key < $maxel) {
				if ($dom[$key]['tag'] OR ($key == 0)) {
					...
					if ((!$this->newline)
						...
						// get image height
						$imgh = $this->gesisgemUnitToUnits($dom[$key]['attribute']['height'], $this->lasth, 'px');
						if (!$this->InFooter) {
							// check for page break
				...
				if ($dom[$key]['tag']) {
					if ($dom[$key]['opening']) {
						if ($dom[$key]['value'] == 'table') {
							...
							if (isset($dom[$key]['width'])) {
								$table_width = $this->gesisgemUnitToUnits($dom[$key]['width'], $wtmp, 'px');
							} else {
								$table_width = $wtmp;
							}
						}
						...
						if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
							...
							if (isset($dom[($dom[$trid]['parent'])]['attribute']['cellpadding'])) {
								$currentcmargin = $this->gesisgemUnitToUnits($dom[($dom[$trid]['parent'])]['attribute']['cellpadding'], 1, 'px');
							} else {
								$currentcmargin = 0;		
							}
							$this->cMargin = $currentcmargin;
							if (isset($dom[($dom[$trid]['parent'])]['attribute']['cellspacing'])) {
								$cellspacing = $this->gesisgemUnitToUnits($dom[($dom[$trid]['parent'])]['attribute']['cellspacing'], 1, 'px');
							} else {
								$cellspacing = 0;
							}
							if ($this->rtl) {
							...
							if (isset($dom[$key]['width'])) {
								$cellw = $this->gesisgemUnitToUnits($dom[$key]['width'], $wtmp, 'px');
							} else {
								$cellw = $wtmp;
							}
							if (isset($dom[$key]['height'])) {
								// minimum cell height
								$cellh = $this->gesisgemUnitToUnits($dom[$key]['height'], 0, 'px');
							} else {
								$cellh = 0;
							}
							$cellw -= $cellspacing;
					...
					if (!$this->empty_string($this->lispacer)) {
						...
						$this->lasth = $this->FontSize * $this->cell_height_ratio;
						$minstartliney = $this->y;
						$this->pusisgemListBullet($this->listnum, $this->lispacer, $pfontsize);
			...
			switch($tag['value']) {
				case 'table': {
					...
					if (isset($tag['attribute']['cellpadding'])) {
						$cp = $this->gesisgemUnitToUnits($tag['attribute']['cellpadding'], 1, 'px');
						$this->oldcMargin = $this->cMargin;
						$this->cMargin = $cp;
					}
					if (isset($tag['attribute']['cellspacing'])) {
						$cs = $this->gesisgemUnitToUnits($tag['attribute']['cellspacing'], 1, 'px');
					}
				...
				case 'hr': {
					...
					if ((isset($tag['attribute']['width'])) AND ($tag['attribute']['width'] != '')) {
						$hrWidth = $this->gesisgemUnitToUnits($tag['attribute']['width'], $wtmp, 'px');
					} else {
						$hrWidth = $wtmp;
					}
					$x = $this->GetX();
				...
				case 'a': {
					...
					if (array_key_exists('style', $tag['attribute'])) {
						...
						if (isset($astyle['color'])) {
							$this->HREF['color'] = $this->conversisgemColorToDec($astyle['color']);
						}
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						$iw = '';
						if (isset($tag['attribute']['width'])) {
							$iw = $this->gesisgemUnitToUnits($tag['attribute']['width'], 1, 'px', false);
						}
						$ih = '';
						if (isset($tag['attribute']['height'])) {
							$ih = $this->gesisgemUnitToUnits($tag['attribute']['height'], 1, 'px', false);
						}
			...
			switch($tag['value']) {
				case 'tr': {
					...
					if (isset($dom[$table_el]['attribute']['cellspacing'])) {
						$cellspacing = $this->gesisgemUnitToUnits($dom[$table_el]['attribute']['cellspacing'], 1, 'px');
						$this->y += $cellspacing;
					}				
					$this->Ln(0, $cell);
				...
				case 'table': {
					...
					foreach ($table_el['trids'] as $j => $trkey) {
						...
						if (isset($table_el['attribute']['cellspacing'])) {
							$cellspacing = $this->gesisgemUnitToUnits($table_el['attribute']['cellspacing'], 1, 'px');
							$this->y += $cellspacing;
						}				
						$this->Ln(0, $cell);
						$this->x = $parent['startx'];
		...
		* @since 4.2.001 (2008-10-30)
		*/
		public function sesisgemVSpace($tagvs) {
		...
		* @since 4.4.003 (2008-12-09)
		*/
		public function sesisgemLinksStyle($color=array(0,0,255), $fontstyle='U') {
		...
		*/
        public function gesisgemUnitToUnits($htmlval, $refsize=1, $defaultunit='px', $points=false) {
		...
		* @since 4.4.004 (2008-12-10)
		*/
		protected function pusisgemListBullet($listdepth, $listtype='', $size=10) {
		    $size /= $this->k;
		    $fill = '';
		    $color = $this->fgcolor;
		    $width = 0;
...
////////////ESTA FUNCION HAY QUE CAMBIARLA CUANDO YA TENGAMOS TODAS LAS NOTAS EN BASE A 100////////////////
   function porcentaje($num=null)
   {
	 // echo "num $num";
	  return ($num *100) / 100;
   }

   function ntab20($num=null)
   {
	 // echo "num $num";
	  if($num > 0 && $num < 6) return 1;
	  else return round($num / 5);
   }
